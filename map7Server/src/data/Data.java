...
public class Data {
	...
	private ContinuosAttribute classAttribute; 
	...
	public Data(String tableName) throws TrainingDataException, ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
		...
		while (it.hasNext()) {
			...
			if (!column.isNumber() && it.hasNext()) {
				...
				explanatorySet.add(new ContinuosAttribute(column.getColumnName(), i));
				i++;
			}

			else if (!it.hasNext() && column.isNumber())
				classAttribute = new ContinuosAttribute(column.getColumnName(), i);
			else
	 ...
	 * 
	 * @param exampleIndex  Indice di posizone all'interno della lista.
	 * @param attributeIndex  Indice di posizione dell'attributo contenuto all'interno della lista explanatorySet.
	 * @return Oggetto associato all'attributo indipendente per l'esempio indicizzato in input.
	 */
	public Object getExplanatoryValue(int exampleIndex, int attributeIndex) { /*Input: indice di riga per la matrice data [][] per uno specifico esempio
	 ...
	 * explanatorySet.
	 * 
	 * @param index  Indice nella lista explanatorySet per uno specifico attributo indipendente.
	 ...
	 */
	ContinuosAttribute getClassAttribute() {																					
	 ...
	 * è dato dai valori assunti dall'attributo passato in input. Vengono richiamati
	 * i metodi: void quicksort(Attribute attribute, int inf, int sup),
	 ...
	 * Partiziona il vettore rispetto all'elemento x e restiutisce il punto di separazione
	 */
	private  int partition(ContinuosAttribute attribute, int inf, int sup){
		int i,j;
	...
	private void quicksort(Attribute attribute, int inf, int sup){
		...
		if(sup>=inf){
			...
			if(attribute instanceof DiscreteAttribute)
				...
				pos=partition((ContinuosAttribute)attribute, inf, sup);
